...
// A person, not a brand: mail from a named sender reads as a letter, not a newsletter.
const FROM = () =>
  process.env.MAIL_FROM || '"Serge from ArteriaStudios" <serge@thearteria.com>'
...
export async function sendRequestReceived(request) {
  try {
    ...
    await smtp.sendMail({
      ...
      text: email.text,
      headers: {
        'X-Entity-Ref-ID': request.id,
        // No open pixel or rewritten links (Brevo): both are marks of bulk mail.

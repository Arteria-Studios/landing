...
/* Content bundled with the site. On the Next.js server /data.js sets
   window.SITE_DATA from the database (admin at /admin) and replaces it. */
...
const SITE = window.SITE_DATA || null;
const PROJECTS = SITE && Array.isArray(SITE.projects) && SITE.projects.length
  ? SITE.projects.map((p) => ({ ...p, home: p.title, img: p.img || "placeholder.webp" }))
  : BUNDLED_PROJECTS;
...
/* Bundled files are named relative to assets/media/; database media are full URLs. */
const mediaSrc = (f) => (/^(https?:)?\/\//.test(f) ? f : f.startsWith("/") ? f.slice(1) : IMG + f);
const media = (p, alt, eager = false) => p.video
  ? `<video muted loop playsinline preload="none" poster="${esc(mediaSrc(p.img))}" data-src="${esc(mediaSrc(p.video))}" aria-label="${esc(alt)}"></video>`
  : `<img src="${esc(mediaSrc(p.img))}" alt="${esc(alt)}" loading="${eager ? "eager" : "lazy"}">`;
...
/* ---------- Home: services from the database ----------
   Each bento tile lists the services of its category (the tile's icon name).
   Popover texts go to services.js through window.SERVICE_TIPS. Tiles with no
   services in the database keep the list written in the HTML. */
if (SITE && Array.isArray(SITE.services) && SITE.services.length) {
  const tips = {};
  document.querySelectorAll(".bento-tile").forEach((tile) => {
    const key = tile.querySelector("[data-icon]")?.dataset.icon;
    const list = tile.querySelector("ul");
    const own = SITE.services.filter((sv) => sv.category === key);
    if (!list || !own.length) return;
    list.innerHTML = own.map((sv) => `<li>${esc(sv.name)}</li>`).join("");
    own.forEach((sv) => { if (sv.description) tips[sv.name] = sv.description; });
  });
  window.SERVICE_TIPS = tips;
}
